<template>
	<view class="u-switch" :class="[value == 2 ? 'u-switch--on' : '', disabled ? 'u-switch--disabled' : '']" @tap="onClick"
	 :style="[switchStyle]">
	...
	export default {
		...
		computed: {
			switchStyle() {
				...
				style.backgroundColor = this.value == 2 ? this.activeColor : this.inactiveColor;
				return style;
			},
			loadingColor() {
				return this.value == 2 ? this.activeColor : null;
			}
		},
		methods: {
			onClick() {
				if (!this.disabled && !this.loading) {
					...
					this.$nextTick(() => {
						this.$emit('change', this.value == 1 ? 1 : 2);
					})
				}
			}
		}
